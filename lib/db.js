...
// Function to retrieve products by search term
export async function getProducts(source, term, page = 1, limit = 20) {
  const offset = (page - 1) * limit;
  const query = `
    SELECT * FROM products
    WHERE source = $1 AND name ILIKE $2
    ORDER BY created_at DESC
    LIMIT $3 OFFSET $4
  `;
  try {
    const result = await pool.query(query, [source, `%${term}%`, limit, offset]);
    return result.rows;
  } catch (e) {
